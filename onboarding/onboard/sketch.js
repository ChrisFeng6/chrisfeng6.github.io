...
function setupGlobal(){
    ...
    timeItemSlideIn = 50;
    t = 0 // global time
    nameList = ["Chris S", "Hannah C", "Me", "Jack M", "Chris F (me)", "John L", "Eric M"]
...
function drawFirst() {
    ...
    master.itemSlideIn(0, 80)
    master.itemSlideIn(1, 250)
...
function drawSecond() {
    ...
    if (t > 600){
    imageDict.arrow.resize(19*unit,0)
...
function drawFifth() {
    ...
    if (t==0) masterScreen.secondInit()
    master.draw(4)
...
function drawSixth() {
    ...
    // options move in
    for (i = 0; i < masterScreen.ideaList.length; i++) {
        var ovalX = parabola("movingToStatic", 75+i*5, 40, t, 652*unit, 104*unit)
        masterScreen.ovalList[i].xPosition = ovalX
        masterScreen.ideaObjectList[i].xPosition = ovalX + (masterScreen.ideaLeftX - 104*unit)
    }
    // options light up
    var ovalFill = parabola("movingToStatic", 170, 5, t, 48, 255)
    var ovalFill2 = parabola("movingToStatic", 190, 5, t, 48, 255)
...
function drawSeventh() {
    ...
    var titleX = parabola("staticToMoving", 20, 40, t, 156*unit, -420*unit)
    masterScreen.screenTitle.xPosition = titleX
    // options move in
    for (i = 0; i < masterScreen.ideaList.length; i++) {
        var ovalX = parabola("staticToMoving", 25+i*5, 40, t, 104*unit, -500*unit)
        masterScreen.ovalList[i].xPosition = ovalX
        masterScreen.ideaObjectList[i].xPosition = ovalX + (masterScreen.ideaLeftX - 104*unit)
    }
    // button
    ...
    // now t =  100
    // init, only once
    if (t == 90) { masterScreen.seventhInit() }
    // tittle move in
    if ( t>= 90 ) {
        var titleX = parabola("movingToStatic", 90, 40, t, 650*unit, 200*unit)
        masterScreen.screenTitle.xPosition = titleX
        masterScreen.seventhDraw()
        // voting results bar growing
        for (i= 0; i < masterScreen.rectList.length; i++) {
            var newWidth = parabola("movingToStatic", 140 + i*10, 40, t, 0, masterScreen.rectWidthList[i])
            masterScreen.rectList[i].w = newWidth
            // diaplay vote number and name of options
            push()
            var alpha = parabola("movingToStatic", 140+i*10, 50, t, 0, 255*0.45)
            noStroke()
            textStyle(NORMAL)
            textSize(24*unit)
            fill(255, alpha)
...
function drawNinth() {
    ...
    for (i = 0; i < masterScreen.bubbleList.length; i++) {
        var alpha = parabola("movingToStatic", 180+30*i, 40, t, 0, 255)
    ...
    imageDict.benefitBar.resize(34*unit, 0)
...
function drawEleventh() {
    ...
    masterScreen.ninthDraw()
    // move the title out 
    var titleX = parabola("staticToMoving", 0, 40, t, 126*unit, 650*unit)
    masterScreen.screenTitle.xPosition = titleX
    // move the bubbles out
    for (i = 0; i < masterScreen.bubbleList.length; i++) {
        var newX = parabola("staticToMoving", 5+5*i, 40, t, masterScreen.bubbleXList2[i], 650*unit)
        masterScreen.bubbleList[i].xPosition = newX
    }
    // move the bars out 
    var barX = parabola("staticToMoving", 30, 40, t, 44*unit, 650*unit)
    imageDict.costBar.resize(522*unit, 0)
    imageDict.benefitBar.resize(34*unit, 0)
    image(imageDict.costBar, barX, 783*unit)
    image(imageDict.benefitBar, barX, 294*unit) 
    // mast move in
    master.moveRight(6, 60)
    if (t<= 150) {
        master.draw(6)
    }
    // The last one move in
    master.itemSlideIn(6,150)
    if (t>150) {
        master.draw(7)
    }
}
